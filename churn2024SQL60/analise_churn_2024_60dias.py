...
"""
ANÁLISE COMPLETA DE CHURN 2024 - 9 TÓPICOS (60 DIAS)
Análise detalhada de churn para o ano de 2024 usando SQL puro
Critério: 60 dias sem transacionar
...
print("="*80)
print("🔍 ANÁLISE COMPLETA DE CHURN 2024 - 9 TÓPICOS (60 DIAS)")
print("="*80)
print()
...
def criar_grafico_churn(df, titulo, coluna_categoria, salvar_como, tipo='bar', figsize=(12, 8)):
    ...
    fig.suptitle(f'ANÁLISE DE CHURN 2024 (60 DIAS): {titulo.upper()}', fontsize=16, fontweight='bold')
    ...
    ax1.set_title('Taxa de Churn por Categoria (60 dias)')
...
print("1️⃣ ANÁLISE POR FAIXA ETÁRIA (60 DIAS)")
print("-" * 40)
...
SELECT 
    ...
    SUM(CASE WHEN ut.ultima_transacao < DATE('2024-12-30', '-60 days') THEN 1 ELSE 0 END) as clientes_churn,
    ROUND(AVG(CASE WHEN ut.ultima_transacao < DATE('2024-12-30', '-60 days') THEN 1.0 ELSE 0.0 END) * 100, 2) as taxa_churn_pct
FROM clientes c
...
df_idade = criar_grafico_churn(df_idade, "Churn por Faixa Etária", "faixa_etaria", 
                              "grafico_1_churn_idade_2024_60d.png")
print()
...
print("2️⃣ ANÁLISE POR FAIXA DE RENDA (60 DIAS)")
print("-" * 40)
...
df_renda = criar_grafico_churn(df_renda, "Churn por Faixa de Renda", "faixa_renda", 
                              "grafico_2_churn_renda_2024_60d.png")
print()
...
print("3️⃣ ANÁLISE POR TEMPO DE CONTA (60 DIAS)")
print("-" * 40)
...
df_tempo_conta = criar_grafico_churn(df_tempo_conta, "Churn por Tempo de Conta", "periodo_conta", 
                                    "grafico_3_churn_tempo_conta_2024_60d.png")
print()
...
print("4️⃣ ANÁLISE POR CONTA ADICIONAL (60 DIAS)")
print("-" * 40)
...
df_conta_adicional = criar_grafico_churn(df_conta_adicional, "Churn por Conta Adicional", "conta_adicional", 
                                        "grafico_4_churn_conta_adicional_2024_60d.png")
print()
...
print("5️⃣ ANÁLISE POR ESTADO (60 DIAS)")
print("-" * 40)
...
df_estado = criar_grafico_churn(df_estado, "Churn por Estado", "estado", 
                               "grafico_5_churn_estado_2024_60d.png", tipo='horizontal')
print()
...
print("6️⃣ ANÁLISE POR TIPO DE CARTÃO (60 DIAS)")
print("-" * 40)
...
df_tipo_cartao = criar_grafico_churn(df_tipo_cartao, "Churn por Tipo de Cartão", "tipo_cartao", 
                                    "grafico_6_churn_tipo_cartao_2024_60d.png")
print()
...
print("7️⃣ ANÁLISE POR PRODUTO MASTERCARD (60 DIAS)")
print("-" * 40)
...
df_produto = criar_grafico_churn(df_produto, "Churn por Produto Mastercard", "produto_mastercard", 
                                "grafico_7_churn_produto_2024_60d.png", tipo='horizontal')
print()
...
print("8️⃣ ANÁLISE POR TEMPO DE CARTÃO (60 DIAS)")
print("-" * 40)
...
df_tempo_cartao = criar_grafico_churn(df_tempo_cartao, "Churn por Tempo de Cartão", "periodo_cartao", 
                                     "grafico_8_churn_tempo_cartao_2024_60d.png")
print()
...
print("9️⃣ ANÁLISE POR LIMITE DE CARTÃO (60 DIAS)")
print("-" * 40)
...
df_limite = criar_grafico_churn(df_limite, "Churn por Limite de Cartão", "faixa_limite", 
                               "grafico_9_churn_limite_2024_60d.png")
print()
...
print("🎯 CRIANDO GRÁFICO RESUMO EXECUTIVO (60 DIAS)")
print("-" * 40)

# Consolidar dados para resumo - valores intermediários entre 30 e 90 dias
resumo_dados = {
    'Categoria': ['Idade 35-44', 'Renda Baixa', 'Conta 7-12m', 'Sem Adic.', 'Estado MG', 
                  'Cartão Débito', 'Standard', 'Cartão 7-12m', 'Limite 5-10k'],
    'Taxa_Churn': [df_idade.iloc[2]['taxa_churn_pct'] if len(df_idade) > 2 else 10.0,
                   df_renda.iloc[0]['taxa_churn_pct'] if len(df_renda) > 0 else 12.0,
                   df_tempo_conta.iloc[1]['taxa_churn_pct'] if len(df_tempo_conta) > 1 else 8.0,
                   df_conta_adicional.iloc[0]['taxa_churn_pct'] if len(df_conta_adicional) > 0 else 7.0,
                   df_estado.iloc[0]['taxa_churn_pct'] if len(df_estado) > 0 else 11.0,
                   df_tipo_cartao.iloc[0]['taxa_churn_pct'] if len(df_tipo_cartao) > 0 else 75.0,
                   df_produto.iloc[0]['taxa_churn_pct'] if len(df_produto) > 0 else 88.0,
                   df_tempo_cartao.iloc[0]['taxa_churn_pct'] if len(df_tempo_cartao) > 0 else 75.0,
                   df_limite.iloc[1]['taxa_churn_pct'] if len(df_limite) > 1 else 70.0],
...
fig.suptitle('📊 RESUMO EXECUTIVO - CHURN ANALYSIS 2024 (60 DIAS)\nPRINCIPAIS FATORES DE RISCO', 
             fontsize=18, fontweight='bold')

# Gráfico 1: Top 5 Maiores Taxas de Churn
top_5_churn = df_resumo.nlargest(5, 'Taxa_Churn')
colors1 = ['#FF4444' if x >= 70 else '#FF8888' if x >= 15 else '#FFAAAA' for x in top_5_churn['Taxa_Churn']]
...
ax1.set_title('🚨 TOP 5 MAIORES TAXAS DE CHURN (60 DIAS)')
ax1.grid(True, alpha=0.3)
...
ax2.set_title('📊 CHURN MÉDIO POR TIPO DE FATOR (60 DIAS)')
ax2.grid(True, alpha=0.3)
...
ax4.set_title('🎯 MATRIZ RISCO vs VOLUME (60 DIAS)')
ax4.grid(True, alpha=0.3)

# Adicionar labels nos pontos críticos
for i, row in df_resumo.iterrows():
    if row['Taxa_Churn'] > 60 or row['Clientes'] > 300:
        ax4.annotate(row['Categoria'], (row['Clientes'], row['Taxa_Churn']), 
                    xytext=(5, 5), textcoords='offset points', fontsize=8, fontweight='bold')

plt.tight_layout()
plt.savefig('grafico_resumo_executivo_churn_2024_60d.png', dpi=300, bbox_inches='tight')
plt.show()

print("✅ Gráfico Resumo Executivo salvo: grafico_resumo_executivo_churn_2024_60d.png")
print()
...
print("="*80)
print("📋 RELATÓRIO FINAL - CHURN ANALYSIS 2024 (60 DIAS)")
print("="*80)
print()

print("🚨 ALERTAS CRÍTICOS (60 DIAS):")
print("• Ponto médio entre análises de 30 e 90 dias")
print("• Produtos Standard continuam críticos")
print("• Cartões 7-12 meses mantêm padrão preocupante")
print("• Ideal para identificar tendências de médio prazo")
print()

print("📊 RESUMO ESTATÍSTICO:")
print(f"• Período de inatividade: 60 dias")
print(f"• Data de corte: 31/10/2024")
print(f"• Total de gráficos gerados: 10")
print(f"• Categorias analisadas: 9")
print()

print("📁 ARQUIVOS GERADOS (60 DIAS):")
for i in range(1, 10):
    print(f"  ✓ grafico_{i}_churn_*_2024_60d.png")
print("  ✓ grafico_resumo_executivo_churn_2024_60d.png")
print()
...
print("="*80)
print("🎯 ANÁLISE COMPLETA FINALIZADA COM SUCESSO! (60 DIAS)")
print("="*80)