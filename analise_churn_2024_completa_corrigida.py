...
"""
Análise de Churn 2024 - Versão Corrigida (90 dias de inatividade)
=================================================================
...
Critério de churn: Sem transações nos últimos 90 dias (desde 2024-10-01)
...
def analise_1_idade_churn(conn):
    """1. Análise de Churn por Faixa Etária - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("1. ANÁLISE DE CHURN POR FAIXA ETÁRIA")
    print("="*60)
    ...
    criar_grafico_churn(
        df, 
        'Análise de Churn por Faixa Etária\n(Todos os churns de 2024 - 90 dias inatividade)',
        'grafico_1_idade_churn_corrigido.png',
        'Faixa Etária'
    )
...
def analise_2_renda_churn(conn):
    """2. Análise de Churn por Faixa de Renda - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("2. ANÁLISE DE CHURN POR FAIXA DE RENDA")
    print("="*60)
    ...
    criar_grafico_churn(
        ...
        'grafico_2_renda_churn_corrigido.png',
        'Faixa de Renda'
    )
...
def analise_3_tempo_conta_churn(conn):
    """3. Análise de Churn por Tempo desde Criação da Conta - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("3. ANÁLISE DE CHURN POR TEMPO DESDE CRIAÇÃO DA CONTA")
    print("="*60)
    ...
    criar_grafico_churn(
        ...
        'grafico_3_tempo_conta_churn_corrigido.png',
...
def analise_4_conta_adicional_churn(conn):
    """4. Análise de Churn por Conta Adicional - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("4. ANÁLISE DE CHURN POR CONTA ADICIONAL")
    print("="*60)
    ...
    criar_grafico_churn(
        ...
        'grafico_4_conta_adicional_churn_corrigido.png',
...
def analise_5_estado_churn(conn):
    """5. Análise de Churn por Estado - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("5. ANÁLISE DE CHURN POR ESTADO")
    print("="*60)
    ...
    LEFT JOIN clientes_churn cc ON tc.cliente_id = cc.cliente_id
    GROUP BY tc.Estado
    HAVING COUNT(*) >= 20  -- Apenas estados com pelo menos 20 clientes
    ORDER BY 5 DESC;
    """
    ...
    criar_grafico_churn(
        ...
        'grafico_5_estado_churn_corrigido.png',
        'Estado'
    )
...
def analise_6_tipo_cartao_churn(conn):
    """6. Análise de Churn por Tipo de Cartão - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("6. ANÁLISE DE CHURN POR TIPO DE CARTÃO")
    print("="*60)
    
    query = """
    WITH todos_clientes AS (
        SELECT DISTINCT t.Cliente_ID as cliente_id, cart.Tipo
        FROM transacoes t
    ...
    criar_grafico_churn(
        df, 
        'Análise de Churn por Tipo de Cartão\n(Todos os churns de 2024 - Dados válidos desde 2023)',
        'grafico_6_tipo_cartao_churn_corrigido.png',
        'Tipo de Cartão'
    )
...
def analise_7_produto_mastercard_churn(conn):
    """7. Análise de Churn por Produto Mastercard - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("7. ANÁLISE DE CHURN POR PRODUTO MASTERCARD")
    print("="*60)
    ...
    criar_grafico_churn(
        ...
        'grafico_7_produto_mastercard_churn_corrigido.png',
        'Produto Mastercard'
    )
...
def analise_8_tempo_cartao_churn(conn):
    """8. Análise de Churn por Tempo desde Emissão do Cartão - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("8. ANÁLISE DE CHURN POR TEMPO DESDE EMISSÃO DO CARTÃO")
    print("="*60)
    ...
    criar_grafico_churn(
        ...
        'grafico_8_tempo_cartao_churn_corrigido.png',
...
def analise_9_limite_cartao_churn(conn):
    """9. Análise de Churn por Limite do Cartão - TODOS os clientes que saíram em 2024"""
    print("\n" + "="*60)
    print("9. ANÁLISE DE CHURN POR LIMITE DO CARTÃO")
    print("="*60)
    ...
    criar_grafico_churn(
        ...
        'grafico_9_limite_cartao_churn_corrigido.png',
        'Faixa de Limite'
    )
...
def resumo_executivo_churn(conn):
    """Gera um resumo executivo completo da análise de churn corrigida"""
    print("\n" + "="*80)
    print("RESUMO EXECUTIVO - ANÁLISE DE CHURN 2024 (VERSÃO CORRIGIDA)")
    print("="*80)
    ...
    print(f"• Clientes em churn (90 dias sem transação): {df_geral.iloc[0]['total_clientes_churn']:,}")
    ...
    print(f"\n🕐 ANÁLISE TEMPORAL DOS CHURNS:")
    for _, row in df_temporal.iterrows():
        print(f"• {row['periodo_criacao']}: {row['clientes_churn']:,} churns ({row['taxa_churn']}%)")
    
    # 3. Top 3 piores taxas de churn por categoria
    print(f"\n⚠️  TOP 3 PIORES TAXAS DE CHURN POR CATEGORIA:")
    ...
    df_idade_top = pd.read_sql_query(query_idade, conn)
    print("   Faixa Etária:")
    for _, row in df_idade_top.iterrows():
        print(f"   - {row['categoria']}: {row['taxa_churn']}%")
    
    return {
        'geral': df_geral,
        'temporal': df_temporal,
        'idade_top': df_idade_top
    }

def main():
    """Função principal que executa todas as análises"""
    print("🚀 INICIANDO ANÁLISE DE CHURN 2024 - VERSÃO CORRIGIDA")
    print("="*80)
    ...
    print("📌 Critério: 90 dias de inatividade desde 01/10/2024")
    ...
    try:
        ...
        print("="*80)
        print("📁 Gráficos salvos com sufixo '_corrigido.png'")
        print("🎯 Esta versão captura o impacto real no marketshare de 2024")
        print("💡 61.6% dos churns são de clientes antigos (conta criada antes de 2024)")
